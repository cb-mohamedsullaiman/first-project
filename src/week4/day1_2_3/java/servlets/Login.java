...
public class Login extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html><body>hi"+userName+password+"</body></html>");
            System.out.println("Before result set in servlet.. ");
            resultSet = DAOImplementation.select("* from users where email = ?", userName);
            out.println("<html><body><p>hi "+userName+" </p></body></html>");
            if (resultSet.next()) {
                resultSet = DAOImplementation.select("* from users where email = ? and password = ?",userName,password);
                if(resultSet.next()){
