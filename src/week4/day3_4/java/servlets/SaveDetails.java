...
public class SaveDetails extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Integer rowsAffected = DAOImplementation.update(" phone_directory_portal.users set firstName = ?, lastName = ?, addressLine1 = ?, addressLine2 = ?, city = ?, zip = ?, state = ?, country = ? where email = ?;",firstName,lastName,addressLine1,addressLine2,city,zip,state,country,email);
            if(rowsAffected!=0){
