			...
			switch(choice){
				case 1:
					...
					do{
						...
						for(Map.Entry<String,ArrayList<Person>> mapEntry : map.entrySet()){
							for(int i=0;i<mapEntry.getValue().size();i++){
								...
								while(iterator.hasNext()){
									numberDetail=(PhoneNumberDetails)iterator.next();
									if(Objects.equals(numberDetail.getPhoneNumber(), personPhoneNumber)){
						...
						numberDetail.setPhoneNumber(personPhoneNumber);
						System.out.println("Is it your\n\t1.mobile number\n\t2.home number\n\t3.work number");
						Integer phoneChoice=scanner.nextInt();
						scanner.nextLine();
						switch(phoneChoice){
							case 1: 
								numberDetail.setType(PhoneNumberDetails.Type.MOBILE);
								if(!person.addPhoneDetails(numberDetail)){
									System.out.println("Phone Number exists");
								}
								break;
							case 2:
								numberDetail.setType(PhoneNumberDetails.Type.HOME);
								if(!person.addPhoneDetails(numberDetail)){
									System.out.println("Phone Number already exists");
								}
								break;
							case 3:
                                                                numberDetail.setType(PhoneNumberDetails.Type.WORK);
								if(!person.addPhoneDetails(numberDetail)){
									System.out.println("Phone Number already exists");
								}
								break;
